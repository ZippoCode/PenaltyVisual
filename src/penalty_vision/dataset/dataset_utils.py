...
def encode_binary_feature(value: str) -> np.ndarray:
...
def encode_metadata(metadata: dict) -> np.ndarray:
    foot = metadata['piede']
    pitch_side = metadata['lato']

    foot_onehot = encode_binary_feature(foot)
    pitch_side_onehot = encode_binary_feature(pitch_side)
