...
class Trainer:
    ...
    def evaluate(self, dataloader, phase='test'):
        ...
        with torch.no_grad():
            for (running_embeds, kicking_embeds, metadata), labels in tqdm(dataloader, desc=f'{phase.capitalize()}'):
                running_embeds = running_embeds.to(self.device)
                kicking_embeds = kicking_embeds.to(self.device)
                metadata = metadata.to(self.device)
                labels = labels.to(self.device)
        ...
        if self.wandb_logger:
            self.metrics_calculator.log_to_wandb(self.wandb_logger, metrics, phase)

        logger.info(f"{phase.capitalize()} Loss: {metrics['loss']:.4f} | Accuracy: {metrics['accuracy']:.4f}")
